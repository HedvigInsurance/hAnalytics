...
// MARK: - iOS, tvOS, Catalyst
  ...
  internal class iOSVendorSystem: VendorSystem {
    ...
    override var manufacturer: String { return "Apple" }
    ...
    override var identifierForVendor: String? { return device.identifierForVendor?.uuidString }
    override var systemName: String { return device.systemName }
    ...
    override var userAgent: String? {
        ...
        if Thread.isMainThread {
          ...
          DispatchQueue.main.sync { userAgent = WKWebView().value(forKey: "userAgent") as? String }
        }
...
// MARK: - watchOS
  ...
  internal class watchOSVendorSystem: VendorSystem {
    ...
    override var manufacturer: String { return "Apple" }
    override var type: String { return "watchos" }
    override var model: String { return deviceModel() }
    override var name: String { return device.model }
    override var identifierForVendor: String? { return device.identifierForVendor?.uuidString }
    override var systemName: String { return device.systemName }
    override var systemVersion: String { device.systemVersion }
    override var screenSize: ScreenSize {
    ...
    override var requiredPlugins: [PlatformPlugin] { return [watchOSLifecycleMonitor()] }
    private func deviceModel() -> String {
      var name: [Int32] = [CTL_HW, HW_MACHINE]
      var size: Int = 2
      sysctl(&name, 2, nil, &size, nil, 0)
...
// MARK: - macOS
  ...
  internal class MacOSVendorSystem: VendorSystem {
    ...
    override var manufacturer: String { return "Apple" }
    override var type: String { return "macos" }
    override var model: String { return deviceModel() }
    override var name: String { return device.hostName }
    ...
    override var systemName: String { return device.operatingSystemVersionString }
    ...
    private func macAddress(bsd: String) -> String? {
      ...
      if bsdIndex == 0 { return nil }
      let bsdData = Data(bsd.utf8)
      var managementInfoBase = [CTL_NET, AF_ROUTE, 0, AF_LINK, NET_RT_IFLIST, bsdIndex]

      if sysctl(&managementInfoBase, 6, nil, &length, nil, 0) < 0 { return nil }

      buffer = [CChar](
        unsafeUninitializedCapacity: length,
        initializingWith: { buffer, initializedCount in for x in 0..<length { buffer[x] = 0 }
          initializedCount = length
        }
      )

      if sysctl(&managementInfoBase, 6, &buffer, &length, nil, 0) < 0 { return nil }
