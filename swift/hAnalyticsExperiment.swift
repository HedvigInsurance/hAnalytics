...
public struct hAnalyticsExperiment {
  ...
  public static func load(onComplete: @escaping (_ success: Bool) -> Void) {
    hAnalyticsNetworking.loadExperiments(
      filter: [
        "allow_external_data_collection", "connect_payment_reminder", "forever",
        "forever_february_campaign", "french_market", "home_common_claim", "key_gear",
        "login_method", "moving_flow", "payment_screen", "payment_type",
        "post_onboarding_show_payment_step", "show_charity", "update_necessary",
        "use_hedvig_letters_font", "use_quote_cart",
      ],
  ...
  public static var connectPaymentReminder: Bool {
    if let experiment = hAnalyticsNetworking.experimentsPayload.first(where: { experiment in
      ...
      hAnalyticsEvent.experimentEvaluated(name: "connect_payment_reminder", variant: variant).send()
      return variant == "enabled"
    }

    hAnalyticsEvent.experimentEvaluated(name: "connect_payment_reminder", variant: "enabled").send()
  ...
  public static var forever: Bool {
    if let experiment = hAnalyticsNetworking.experimentsPayload.first(where: { experiment in
      ...
      hAnalyticsEvent.experimentEvaluated(name: "forever", variant: variant).send()
      return variant == "enabled"
    }

    hAnalyticsEvent.experimentEvaluated(name: "forever", variant: "enabled").send()
  ...
  public static var homeCommonClaim: Bool {
    if let experiment = hAnalyticsNetworking.experimentsPayload.first(where: { experiment in
      ...
      hAnalyticsEvent.experimentEvaluated(name: "home_common_claim", variant: variant).send()
      return variant == "enabled"
    }
  ...
  public static var paymentScreen: Bool {
    if let experiment = hAnalyticsNetworking.experimentsPayload.first(where: { experiment in
      ...
      hAnalyticsEvent.experimentEvaluated(name: "payment_screen", variant: variant).send()
      return variant == "enabled"
    }

    hAnalyticsEvent.experimentEvaluated(name: "payment_screen", variant: "enabled").send()
  ...
  public static var showCharity: Bool {
    if let experiment = hAnalyticsNetworking.experimentsPayload.first(where: { experiment in
      ...
      hAnalyticsEvent.experimentEvaluated(name: "show_charity", variant: variant).send()
      return variant == "enabled"
    }

    hAnalyticsEvent.experimentEvaluated(name: "show_charity", variant: "enabled").send()
