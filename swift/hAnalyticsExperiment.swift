...
public static func load(onComplete: @escaping (_ success: Bool) -> Void) {
    hAnalyticsNetworking.loadExperiments(filter: ["allow_external_data_collection","enable_adyen_apple_pay","forever_february_campaign","french_market","key_gear","login_method","moving_flow","payment_type","post_onboarding_show_payment_step","update_necessary"], onComplete: onComplete)
}
    ...
    public static var enableAdyenApplePay: Bool {
       if let experiment = hAnalyticsNetworking.experimentsPayload.first(where: { experiment in
            experiment["name"] == "enable_adyen_apple_pay"
       }), let variant = experiment["variant"] {
            hAnalyticsEvent.experimentEvaluated(
                name: "enable_adyen_apple_pay",
                variant: variant
            ).send()
