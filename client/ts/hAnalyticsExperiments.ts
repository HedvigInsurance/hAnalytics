...
export class hAnalyticsExperiments {
    ...
    async load(): Promise<hAnalyticsExperiment[]> {
        const list = await this.networking.loadExperiments(
            ["allow_external_data_collection","forever_february_campaign","french_market","key_gear","login_method","moving_flow","payment_type","post_onboarding_show_payment_step","update_necessary","use_hedvig_letters_font","use_quote_cart"]
        )
        this.loading = false
        return list
    }
