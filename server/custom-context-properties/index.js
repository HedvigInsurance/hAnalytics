...
const getContractTypesFromProductPricing = async (headers) => {
    const contractsResponse = await fetch(process.env.API_GATEWAY_ENDPOINT + '/productPricing/contracts', {
        method: 'GET',
        headers,
    })
    const contractsJson = await contractsResponse.json()
    return contractsJson
        .map(c => c.typeOfContract)
}
...
const isNonPayingMember = async (headers) => {
    ...
    return !contractTypes.some(type => !NON_PAYING_CONTRACT_TYPES.includes(type))
}
